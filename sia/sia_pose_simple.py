...
class SIA_POSE_SIMPLE(nn.Module):
    """
    Simple SIA Pose Model with direct keypoint regression from encoder tokens.
...
# ============================================================================
# Decoder variant: pose tokens processed by lightweight cross-attention decoder
# ============================================================================

class VisionTransformerSimpleDecoder(VisionTransformer):
    """
    Vision Transformer with lightweight pose decoder.

    Detection tokens go through the full ViT encoder (unchanged).
    Pose tokens are NOT in the encoder — instead they are processed by a
    lightweight decoder that cross-attends to the encoder's spatial features.

    Architecture:
        Encoder (ViT): [image_patches + det_tokens] -> spatial_features + det_x
        Pose Decoder (N layers): pose_queries + cross-attn(spatial_features) -> pose_x
        Det Heads: det_x -> bbox, human
    ...
    def __init__(
        ...
        super().__init__(
            ...
            det_token_num=det_token_num,
            num_keypoints=num_keypoints,
            pose_decoder_layers=0,   # No parent pose decoder
            enable_pose=False,       # Disable parent's pose handling
        )
    ...
    def forward(self, x, masking_prob=0.0):
        """Forward pass — encoder processes patches+det, decoder processes pose queries."""
        _, _, _, in_H, in_W = x.shape
        x = self.conv1(x)
        B, C, T, H, W = x.shape
        x = x.permute(0, 2, 3, 4, 1).reshape(B * T, H * W, C)
        ...
        # [DET] tokens go into encoder (NO pose tokens here)
        if self.det_token_num > 0:
            det_tokens = self.det_token + self.det_positional_embedding
            det_tokens = det_tokens.unsqueeze(0).expand(B, -1, -1)
            x = torch.cat((x, det_tokens), dim=1)
        ...
        # Split spatial features and det tokens from encoder output
        if self.det_token_num > 0:
...
def clip_joint_b16_simple_dec(
    pretrained=False, input_resolution=224, kernel_size=1,
    center=True, num_frames=9, drop_path=0., checkpoint_num=0, det_token_num=100,
    dropout=0., num_keypoints=17, decoder_layers=3,
):
    """ViT-B/16 with lightweight pose decoder."""
    model = VisionTransformerSimpleDecoder(
        input_resolution=input_resolution, patch_size=16,
        width=768, layers=12, heads=12, output_dim=512,
        kernel_size=kernel_size, num_frames=num_frames,
        checkpoint_num=checkpoint_num,
...
def clip_joint_l14_simple_dec(
    pretrained=False, input_resolution=224, kernel_size=1,
    center=True, num_frames=9, drop_path=0., checkpoint_num=0, det_token_num=100,
    dropout=0., num_keypoints=17, decoder_layers=3,
):
    """ViT-L/14 with lightweight pose decoder."""
    model = VisionTransformerSimpleDecoder(
        input_resolution=input_resolution, patch_size=14,
        width=1024, layers=24, heads=16, output_dim=768,
        kernel_size=kernel_size, num_frames=num_frames,
...
class SIA_POSE_SIMPLE_DEC(nn.Module):
    ...
    to encoder spatial features (similar to ED-Pose / PETR).
    """
    def __init__(self,
                 size='l',
                 pretrain=os.path.join(os.path.dirname(os.path.abspath(__file__)), "ViClip-InternVid-10M-FLT.pth"),
        ...
        if self.det_token_num > 0:
            print(f'Type: [DET] regression (SIMPLE-DEC, {decoder_layers} decoder layers)')
        else:
            print(f'Type: [PATCH] regression (SIMPLE-DEC, {decoder_layers} decoder layers)')

        print(f'Simple-Dec pose detection: {num_keypoints} keypoints, {decoder_layers} decoder layers')
    ...
    def forward(self, video):
        """Forward pass for pose estimation with lightweight decoder."""
        return self.encode_vision(video)
    ...
    def build_vision_encoder(self):
        """Build vision encoder with lightweight pose decoder."""
        encoder_name = self.vision_encoder_name
        if encoder_name == "vit_l14":
            vision_encoder = clip_joint_l14_simple_dec(
                pretrained=self.vision_encoder_pretrained,
                input_resolution=self.inputs_image_res,
                kernel_size=self.vision_encoder_kernel_size,
                center=self.vision_encoder_center,
            ...
            vision_encoder = clip_joint_b16_simple_dec(
                pretrained=self.vision_encoder_pretrained,
                input_resolution=self.inputs_image_res,
                kernel_size=self.vision_encoder_kernel_size,
                center=self.vision_encoder_center,
